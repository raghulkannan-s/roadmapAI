...
export default function Loading({ message = "Loading..." }) {
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      ...
      <div className="relative z-10 flex flex-col items-center space-y-6 sm:space-y-8 px-4 text-center">
        {/* Logo and title */}
...
export function LoadingSpinner({ size = "md", color = "emerald" }) {
  const sizeClasses = {
    ...
    lg: "w-10 h-10 sm:w-12 sm:h-12 border-2"
  };
...
export function LoadingCard() {
  return (
    <div className="bg-slate-900/50 backdrop-blur border border-slate-800 rounded-lg p-4 sm:p-6 animate-pulse">
      <div className="flex justify-between items-center mb-3 sm:mb-4">
        <div className="h-3 sm:h-4 bg-slate-800 rounded w-1/3"></div>
        <div className="h-2 sm:h-3 bg-slate-800 rounded w-1/4"></div>
      </div>
      <div className="h-2 sm:h-3 bg-slate-800 rounded w-full mb-2"></div>
      <div className="h-2 sm:h-3 bg-slate-800 rounded w-2/3 mb-4"></div>
      <div className="h-6 sm:h-8 bg-slate-800 rounded w-full"></div>
    </div>
  );
}
...
export function LoadingOverlay({ message = "Processing..." }) {
  return (
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-slate-900 border border-slate-800 rounded-lg p-6 sm:p-8 flex flex-col items-center space-y-3 sm:space-y-4 max-w-xs sm:max-w-sm mx-4">
