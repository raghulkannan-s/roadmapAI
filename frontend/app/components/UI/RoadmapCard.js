  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg transition-shadow p-4 flex flex-col justify-between h-full">
      ...
      <div className="flex justify-between items-start mb-3">
        <h2 className="text-lg font-semibold text-gray-900 line-clamp-2" title={data.title}>
          {data.title}
        </h2>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {data.estimated_duration && (
          <span className="text-xs bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-xs">
            Duration: {data.estimated_duration}
          </span>
        )}
        {data.segments?.length && (
          <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded-xs">
